...
if (isset($_POST["signup"])) {
    ...
    if ($pass == $cpass) {
        ...
        $CheckQuery = "SELECT * FROM `cpr_tester` WHERE `full_name`='$na' AND `email`='$em'";
        $CheckResult = mysqli_query($conn, $CheckQuery);

        if (mysqli_num_rows($CheckResult) == 0) {
            $query = "INSERT INTO `cpr_tester`(`full_name`, `email`, `education`, `contact_number`, `work_experience`,`salary`, `skills`, `portfolio_url`, `comments`, `country`, `category_id`, `password`, `status`) 
